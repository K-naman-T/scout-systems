...
async def applications(request: Request):
    """Application tracker view — reads apply-agent/tracker.csv."""
    import csv as _csv
    tracker = Path(__file__).parent.parent / "apply-agent" / "tracker.csv"
    rows = []
    if tracker.exists():
    ...
    def badge(verdict: str) -> str:
        color = {"APPLY": "#4ade80", "FLAG": "#fbbf24", "DISCARD": "#f87171"}.get(verdict, "#94a3b8")
        return f'<span style="color:{color};font-weight:700">{verdict}</span>'

    trs = "".join(
        f"<tr><td style='padding:8px 12px;border-bottom:1px solid #2a2f3a'>{r.get('timestamp','')[:19]}</td>"
        f"<td style='padding:8px 12px;border-bottom:1px solid #2a2f3a'><b>{r.get('company','')}</b><br>"
        f"<span style='color:#94a3b8'>{r.get('role','')}</span></td>"
        f"<td style='padding:8px 12px;border-bottom:1px solid #2a2f3a'>{badge(r.get('verdict',''))}</td>"
        f"<td style='padding:8px 12px;border-bottom:1px solid #2a2f3a'>{r.get('status','')}</td>"
        f"<td style='padding:8px 12px;border-bottom:1px solid #2a2f3a'>{r.get('role_family','')}</td>"
        f"<td style='padding:8px 12px;border-bottom:1px solid #2a2f3a;color:#94a3b8'>{r.get('top_keywords','')[:60]}</td>"
        f"<td style='padding:8px 12px;border-bottom:1px solid #2a2f3a'><a href='{r.get('apply_url','')}' "
        f"style='color:#60a5fa' target='_blank'>apply ↗</a></td></tr>"
        for r in rows
    )

    html = f"""<!DOCTYPE html><html><head><title>Applications — Tracker</title></head>
<body style="margin:0;background:#0f1115;color:#e5e7eb;font-family:system-ui,sans-serif">
<div style="max-width:1100px;margin:0 auto;padding:24px">
<h1 style="font-size:20px">Applications</h1>
<p style="color:#94a3b8">Generated by apply-agent (tracker.csv). Human-gated — nothing auto-submitted.</p>
<table style="width:100%;border-collapse:collapse;font-size:13px;background:#171a21;border-radius:8px;overflow:hidden">
<thead><tr style="text-align:left;color:#94a3b8;border-bottom:2px solid #2a2f3a">
<th style="padding:10px 12px">When</th><th style="padding:10px 12px">Company / Role</th>
<th style="padding:10px 12px">Verdict</th><th style="padding:10px 12px">Status</th>
<th style="padding:10px 12px">Family</th><th style="padding:10px 12px">Keywords</th>
<th style="padding:10px 12px">Link</th></tr></thead><tbody>{trs}</tbody></table>
<p style="color:#94a3b8;margin-top:16px;font-size:12px">{len(rows)} tracked applications</p>
</div></body></html>"""
